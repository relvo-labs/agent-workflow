...
classification, exact identity, candidate SHA, DAG and acceptance IDs.
...
records to the actual PR. A receipt cannot silently change task/run/session.

## Identity, DAG and leases

One provider/session MUST bind one repository/worktree/branch/base/issue/PR tuple.
...
handoff. The from/to PR and repository remain the same. One writer is governance,
...
quiescence, exact candidate match and passing evidence for every acceptance ID.
...
rule; the reviewer decides whether observations are current enough.
